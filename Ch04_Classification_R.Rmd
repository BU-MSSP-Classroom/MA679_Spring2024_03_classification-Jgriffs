...
```{r,echo=TRUE}
#basic
Model0<-glm(data=Train_COVID,SURVIVED_bin ~ 1,family = "binomial")

#age seems important, as demonstrated toward the top of the doc
Model1<-glm(data=Train_COVID,SURVIVED_bin ~ AGE_TRUNK,family = "binomial")

#admit
ggplot(Train_COVID,aes(PATIENT_TYPE))+geom_histogram() ###admit/not admit too

##age + admit + other things that seem to be risk factors
Model2<-glm(data=Train_COVID,SURVIVED_bin ~ OBESITY + ASTHMA + AGE_TRUNK + COPD + PATIENT_TYPE + FEMALE + MEDICAL_UNIT,family = "binomial")

Model3<-glm(data=Train_COVID,SURVIVED_bin ~ + ASTHMA + AGE_TRUNK + PATIENT_TYPE + FEMALE + MEDICAL_UNIT,family = "binomial")
...
###plots
binnedplot(predict(Model1,type="response"),resid(Model1,type="response" ))
binnedplot(predict(Model2,type="response"),resid(Model2,type="response" ))
binnedplot(predict(Model3,type="response"),resid(Model3,type="response" ))

```


Your answer: I will choose age, sex, and admit status, along with some various health factors.
...
#Model2
lgMod2<- predict(Model2,Covid_Data,type="response")
Pred2<- ifelse(lgMod2 > 0.5,1,0)
Confuse2<-table(Covid_Data$SURVIVED_bin,Pred2)
Confuse2

library(pROC)
roc_scoreMod2=roc(Covid_Data$SURVIVED_bin, lgMod2) #AUC score
plot(roc_scoreMod2, main ="Mod2 ROC")


#Model3
lgMod3<- predict(Model3,Covid_Data,type="response")
Pred3<- ifelse(lgMod3 > 0.5,1,0)
Confuse3<-table(Covid_Data$SURVIVED_bin,Pred3)
Confuse3

roc_scoreMod3=roc(Covid_Data$SURVIVED_bin, lgMod3) #AUC score
plot(roc_scoreMod3, main ="Mod3 ROC")
```

Your answer:

Model 2 and 3 performed fairly well and pretty close to each other. 
...
Your answer: I chose age and admit, since they seemed obviously related, and a handful of other health factors that I thought (based on news and personal experience) were risk factors for covid hospitalization/severity. These additional health factors did not seem to matter much because when I removed them, the resulting model performed just about the same.
...
Your answer: Patient type, age, and hospitalization type seem to be most important.
...
mpgmedian<-median(Auto$mpg,na.rm = TRUE)
...
```{r}
###cylinders
ggplot(Auto,aes(cylinders,mpg))+geom_boxplot(aes(group=cylinders))+geom_hline(yintercept = mpgmedian)

###horsepower
ggplot(Auto,aes(horsepower,mpg))+geom_point(aes(color="Pink"))+geom_hline(yintercept = mpgmedian)

###weight
ggplot(Auto,aes(weight,mpg))+geom_point(aes(color=cylinders))+geom_hline(yintercept = mpgmedian)
...
Your code:
```{r,echo=TRUE}
Train_Auto<-slice_sample(Auto,n=98)
Test_Auto<-anti_join(Auto,Train_Auto)
```

Your answer: Split using slice sample and anti join. 75 percent in train, 25 percent in test.
...
LDA_MPG1<-lda(mpg01~horsepower+weight+cylinders,data=Train_Auto)##subset part wasnt working?
LDA_MPG1


Error_Auto<-predict(LDA_MPG1,newdata = Test_Auto)

#b)

Error_Rate_Auto<- mean(Error_Auto$class!=Test_Auto$mpg01)

```

Your answer: .1054 is the error rate using LDA.
...
Your code:
```{r,echo=TRUE}
qda_MPG <- qda(mpg01~horsepower+weight+cylinders,data=Train_Auto)
qda_MPG

QDA_Auto_Error<-predict(qda_MPG,newdata=Test_Auto)

QDA_Error_Rate_Auto<-mean(QDA_Auto_Error$class!=Test_Auto$mpg01)

QDA_Error_Rate_Auto

```

Your answer: The error rate with QDA is 0.0816
...
```{r,echo=TRUE}
MPG01GLM<-glm(mpg01~horsepower+weight+cylinders,data=Train_Auto,family="binomial")

LR_predict<-predict(MPG01GLM,newdata =Test_Auto,type = "response")

LR_predict<-as.data.frame(LR_predict)

LR_predict$MPG01_class_predict<-ifelse(LR_predict$LR_predict > mpgmedian,1,0)

###Error

LR_Error_Rate_Auto<-mean(PREDS$class!=Test_Auto$mpg01)

LR_Error_Rate_Auto

#-------------------------------------------------------------

MPG01GLM_2<-glm(mpg01~horsepower,data=Train_Auto,family="binomial")

LR2_predict<-predict(MPG01GLM_2,newdata =Test_Auto,type="response")

LR2_predict<-as.data.frame(LR2_predict)

LR2_predict$class_predict<-ifelse(LR2_predict$LR2_predict > mpgmedian,1,0)


#-------------------------------------------------------------

MPG_Null<-glm(mpg01~1,data=Train_Auto,family="binomial")

Null_Predict_Data<-predict(MPG_Null,newdata=Train_Auto,type="response")

Null_Predict_Data<-as.data.frame(Null_Predict_Data)

Null_Predict_Data$Class_predict<-ifelse(Null_Predict_Data$Null_Predict_Data > mpgmedian,1,0)

unique(Null_Predict_Data$Class_predict)
...
Your answer:Not sure what happened here. Looks like MPGs were way underestimated?
...
AutoBayes<- naiveBayes(mpg01~horsepower+weight+cylinders,data=Train_Auto)

AutoBayes
```

Your answer:I don't think I understand this. I can make the code work (somewhat) but don't fully understand. Would be useful to go over in class. I know we haven't quite covered yet.
###!
...
library(class)
set.seed(42)
# train.direction=Direction(Auto$mpg)
# KNN_Auto<- knn(Train_Auto,Test_Auto, train.Direction, k = 1)

```

Your answer: I don't quite understand how to do this. I looked at the lab file but can't figure how it translates.
